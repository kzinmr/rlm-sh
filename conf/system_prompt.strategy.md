...
Approach (general guidance, no recipes):
- Prefer deterministic shell tools (search, slice, split, count) to inspect and narrow
  the input before spending LLM calls.
- If the input is too large to read or process at once, break it into pieces, handle each
  piece, then combine the partial results.
- Keep any parallelism modest.
