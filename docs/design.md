...
| `context` 変数（in-memory） | **`/work/context/`** 配下のファイル群 + `/work/context.txt` | 巨大入力はファイルとして配置 |
...
| RESERVED_TOOL_NAMES（上書き禁止） | **予約ファイル/パス**（`answer.txt`, `context/`） を毎ステップ復元 | §9 |
...
```
   ...
   └──────────────┘   ┌──────────────────────────────────────────────┐
                      ...
                      │                                                │
                      │   /work/  (= メモリ / filesystem)              │
                      │     context/(ro)  answer.txt  history.md       │
...
- **既定の守り（安く効く2点・M1 必須）**:
  ...
  2. **Docker 使い捨て + 資源制限**（`--rm` + `/work` だけマウント、`--pids-limit`/`--memory`/`--cpus`、`docker exec` に `timeout`）。
...
- ワークスペース: `/work` をホストの一時ディレクトリにマウント（`-v tmpdir:/work`）。これが「メモリ」。
...
# simonw/llm + 必要プラグイン
RUN pipx install llm && pipx ensurepath
# プロバイダプラグイン（Anthropic/Gemini を使う場合）
RUN ~/.local/bin/llm install llm-anthropic llm-gemini || true
...
`/work` のレイアウト規約（モデルにシステムプロンプトで提示）:

```
/work/
  context/         # 入力（巨大コンテキスト）。読み取り専用扱い
...
- **context は immutable**: 既定は **`-v ...:/work/context:ro`（read-only マウント）** で破壊を構造的に防ぐ（§9.2）。毎ステップのハッシュ照合・復元は M4 の任意強化。
...
| 観測 | `llm logs -d "$ROOT_DB" -n 50 --json` / `llm logs path` | **全呼び出しが SQLite に自動記録** |
...
docker run --rm -e OPENAI_API_KEY="$RLMSH_KEY" \
  ...
  -v "$WORK_HOST:/work" rlm-sandbox tail -f /dev/null
...
ROOT_CID=$(llm -d "$ROOT_DB" logs -n 1 --json | jq -r '.[0].conversation_id')
...
- root 会話・コスト・トークンは `ROOT_DB` に丸ごと残り、`llm -d "$ROOT_DB" logs --json` で観測（§10.2）。
...
- 環境の説明: 「あなたは bash を使える。`/work/context/` に巨大な入力がある。全部は一度に読めない。」
...
3. **snapshot して渡す（TOCTOU 回避）**: 検証通過後、その時点のファイルを**子ワークスペースへコピー（snapshot）**してから子 sandbox を起動する。子は親 `/work` を共有せず、自分の `/work/context/` に複製を持つ。検証時刻と使用時刻の競合（TOCTOU）を構造的に排除。
...
- **② Docker 使い捨て + リソース制限**: `--rm`、`/work` のみマウント（他のホスト FS を触らせない）、`--pids-limit`（fork bomb 対策）、`--memory`/`--cpus`、`docker exec` に `timeout`。誤った `rm -rf` も `/work` 内に限定され run 後に破棄。
...
|---|---|---|
| **P-min（最小）** | 「bash が使える。`/work/context/` に入力。`llm` でサブ呼び出し可。`/work/answer.txt` に答えを書け」だけ。戦略は一切示さない | **真の自発性**。grep/split/MapReduce が出れば H1 の強い証拠 |
...
rlm-sh/
  ...
  conf/
    system_prompt.md     # ルート LM 用システムプロンプト（bash 版）
  bin/
    rlm-sh               # サンドボックス内 thin client（再帰 spawn を Host に依頼）
    submit               # answer.txt 書き込み（FINAL）
  host/
    orchestrator.py      # run 起動 / answer 検出 / ログ収集 / 子sandbox spawn・depth(§7)
    sandbox.py           # Docker 起動/実行/cleanup（docker_repl.py を bash 化）
    env_base.py          # BaseEnv 抽象（Docker/E2B/local 差し替え）
    loop_shell.sh        # ルートループ A: pure-shell（--cid 固定・ホスト実行）
    harness.py           # ルートループ C: 薄い Python ハーネス（任意）
  tasks/
    niah.py              # NIAH 生成（rlm-minimal/main.py 移植）
    patents/             # cheat-at-search 移植
  README.md
...
- **M1 — pure-shell ループ + NIAH（最低限ガードレール込み）**: `host/loop_shell.sh` で **`--cid` 固定**の root ループ。Docker sandbox で bash 実行・切り詰め・`answer.txt` 検出（ホスト path）。NIAH を解かせ RQ1（grep で解くか）を観察。
  - **M1 から必須のガードレール（§9.1）**（後回しにしない・ただし安いものだけ）: **① 予算上限付き専用キー**、**② `--rm` + `/work` のみマウント + `--pids-limit`/`--memory`/`--cpus` + `docker exec` の `timeout`**、**`context/` の `:ro` マウント**、ループ側の `llm` 呼び出し回数上限。proxy/egress allowlist は不要。
...
[iter 1] root LM (gpt-5):
  ```bash
  ls -la /work/context/ && wc -l /work/context/context.txt
  ```
  → stdout: context.txt  1000000 lines  (切り詰め返却)

[iter 2] root LM:
  ```bash
  rg -n "magic number" /work/context/context.txt | head
  ```
...
```
[iter 1] ```bash
  wc -c /work/context/*.md
  split -n l/20 /work/context/big.md /work/chunks/c_   # 20分割
  ```
[iter 2] ```bash
