...
* **M6 (マルチ環境対応)**: `--backend docker|local-unsafe|docker-sandboxes` の backend 選択を導入。Docker、Docker Sandboxes、local-unsafe は実装済み。Docker Sandboxes は `sandbox.py build --backend docker-sandboxes` で Docker image を `sbx` template store へ load してから使う。
